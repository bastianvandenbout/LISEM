...
inline std::vector<cTMap *> ConductiveFeedbackDiffuseDuo(cTMap * dataI,cTMap * dataI2, cTMap * conductivitydata, cTMap * sigma, cTMap * fbweight, int iter_max, float timestep)
{
    ...
    cTMap * data2= dataI2->GetCopy();
    cTMap * dataN2= dataI2->GetCopy();
        ...
        while( true)
        {
            ...
            for(int r = 0; r < data->data.nr_rows();r++)
            {
                for(int c = 0; c < data->data.nr_cols();c++)
                {
                    if(!pcr::isMV(data->data[r][c]))
                    {
                        ...
                        if(!pcr::isMV(data2->data[r][c]))
                        {
                            float prop = data2->Drc;
                            float propx1 = !OUTORMV(data2,r,c-1)? data2->data[r][c-1] : prop;
                            float propx2 = !OUTORMV(data2,r,c+1)? data2->data[r][c+1] : prop;
                            float propy1 = !OUTORMV(data2,r-1,c)? data2->data[r-1][c] : prop;
                            float propy2 = !OUTORMV(data2,r+1,c)? data2->data[r+1][c] : prop;
                            ...
                            if(std::isnormal(datan))
                            {
                                dataN2->data[r][c] = fbweight->data[r][c] * data2->data[r][c] + (1.0- fbweight->data[r][c]) * datan;
                            }else
                            {
                                dataN2->data[r][c] = data2->data[r][c];
                            }
                        }
            ...
            for(int r = 0; r < data->data.nr_rows();r++)
            {
                for(int c = 0; c < data->data.nr_cols();c++)
                {
                    ...
                    {
                        data->data[r][c] =dataN->data[r][c];
                        data2->data[r][c] =dataN2->data[r][c];
                    }
                }
            }
        }
