...
inline cTMap * AS_DiffusiveMaxWave(cTMap * DEM,cTMap * H, int iter, float couranti)
{
    ...
    if(couranti < 0.0 || couranti > 1.0f)
    {
        LISEMS_ERROR("Input 4 for DiffusiveMaxWave (courant factor) must be between 0.0 and 1.0. Current value: "+ QString::number(couranti));
        throw 1;
    }
    ...
    cTMap *map2 = new cTMap(std::move(raster_data2),DEM->projection(),"");
