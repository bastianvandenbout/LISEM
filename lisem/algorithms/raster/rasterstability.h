...
inline static void Sample_Slope1(cTMap * DEM, float &s_x1, float &s_x2, float &s_y1, float &s_y2, int radius_pixels_x, int radius_pixels_y, int r, int c, float dx, float dy)
{
        ...
        for(int r2 = rmin; r2 < rmax; r2++)
        {
            for(int c2 = cmin; c2 < cmax; c2++)
            {
                ...
                if(!pcr::isMV(z))
                {
                    ...
                    Sample_Slope1(DEM,s2_x1,s2_x2,s2_y1,s2_y2,radius_pixels_x,radius_pixels_y,r,c,dx,dy);
                    ...
                    if(weight > 1e-6)
                    {
                        den_total += std::max(0.01,(coh_this * Area + tan(ifa_this) * std::max(0.0001,(weight-wz_z * 1000.0 * dx * dy)) * cos(betac) - seep_c)) * cos(betam);
...
inline static std::vector<std::vector<cTMap*>> AS_SlopeStabilityRESAll(cTMap * DEM, std::vector<cTMap*> soildepth, std::vector<cTMap*> coh, std::vector<cTMap*> ifa, std::vector<cTMap*> dens, std::vector<cTMap*> sat, cTMap * WH,
                                                 float sample_density, float h_min, float h_max, float size_min, float size_max, float size_lat_min, float size_lat_max, float size_vert_min, float size_vert_max,
                                                 float rot_min, float rot_max, float rot_lat_min, float rot_lat_max, float vol_min)
{

    cTMap * TrackIndices;
    cTMap * TrackIndicesUS;
    std::vector<cTMap *> TrackElevation;
    std::vector<cTMap *> TrackVolume;
    std::vector<cTMap *> TrackVolumeIntersect;
    std::vector<cTMap *> TrackDeepestPoint;
    std::vector<cTMap *> TrackArea;
    std::vector<cTMap *> TrackFoS;

    TrackIndices = DEM->GetCopy0();
    TrackIndicesUS = DEM->GetCopy0();
    int max_index = ((int)(sample_density +1.0));

    for(int i = 0; i < max_index; i++)
    {
        TrackElevation.push_back(DEM->GetCopy0());
        TrackVolume.push_back(DEM->GetCopy0());
        TrackVolumeIntersect.push_back(DEM->GetCopy0());
        TrackDeepestPoint.push_back(DEM->GetCopy0());
        TrackArea.push_back(DEM->GetCopy0());
        TrackFoS.push_back(DEM->GetCopy0());
    }
    ...
    for(int r = 0; r < DEM->data.nr_rows();r++)
    {
        for(int c = 0; c < DEM->data.nr_cols();c++)
        {
            if(!pcr::isMV(DEM->data[r][c]))
            {
                TrackIndices->data[r][c] = 0.5f;
                TrackIndicesUS->data[r][c] = 0.5f;
            }
        }
    }
...
#pragma omp parallel for collapse(1)
    for(int i = 0; i < n_ellips; i++)
    {
        ...
        for(int r2 = rmin; r2 < rmax; r2++)
        {
            for(int c2 = cmin; c2 < cmax; c2++)
            {
                ...
                float z = elev + h + Sample_Ellipsoid_Z_Bottom(x,y,size,size_lat,size_vert,-aspect+rot_lat,angle + rot);
                float z_x1 = elev + h + Sample_Ellipsoid_Z_Bottom(x- 0.1,y,size,size_lat,size_vert,-aspect+rot_lat,angle + rot);
                float z_x2 = elev + h + Sample_Ellipsoid_Z_Bottom(x+ 0.1,y,size,size_lat,size_vert,-aspect+rot_lat,angle + rot);
                float z_y1 = elev + h + Sample_Ellipsoid_Z_Bottom(x,y-0.1,size,size_lat,size_vert,-aspect+rot_lat,angle + rot);
                float z_y2 = elev + h + Sample_Ellipsoid_Z_Bottom(x,y+0.1,size,size_lat,size_vert,-aspect+rot_lat,angle + rot);
                ...
                if(!pcr::isMV(z))
                {
                    ...
                    {
                        //m.lock();
                        //std::cout << "z " << z << " " << DEMBottom->data[r][c] << " " << height_total << std::endl;
                        //std::cout << "ellipsoid " << coh_this << " " << ifa_this << " " << weight << " " << betam << " " << betac << " " << sz_x1 << "  " << sz_y1 <<  " " << s2_x1 << "  " << s2_y1 <<  std::endl;
                        //m.unlock();
        ...
        if(div_total > 0.0 && volume > vol_min)
        {
            ...
            if(FoS > 1.0)
            {
                ...
                m.lock();
                int index = std::max(0,((int)(TrackIndices->data[r][c])));

                if(index > -1 && index < max_index )
                {
                    TrackElevation[index]->data[r][c] = h;
                    TrackVolume[index]->data[r][c] = volume;
                    TrackVolumeIntersect[index]->data[r][c] = (4.0/3.0) * 3.14 * size * size_lat * size_vert;
                    TrackDeepestPoint[index]->data[r][c] = z_min;
                    TrackArea[index]->data[r][c] = area;
                    TrackFoS[index]->data[r][c] = FoS;
                }

                TrackIndices->data[r][c] += 1.0;
            ...
            }else if(FoS < 1.0)
            {
                m.lock();
                int index = std::max(0,((int)(TrackIndices->data[r][c])));
                int indexus = std::max(0,((int)(TrackIndicesUS->data[r][c])));

                //if(index > -1 && index < max_index )
                {

                    if(index > indexus && index < max_index && index > -1 && indexus > -1 && indexus < max_index)
                    {
                        TrackElevation[index]->data[r][c] = TrackElevation[indexus]->data[r][c];
                        TrackVolume[index]->data[r][c] = TrackVolume[indexus]->data[r][c];
                        TrackVolumeIntersect[index]->data[r][c] = TrackVolumeIntersect[indexus]->data[r][c];
                        TrackDeepestPoint[index]->data[r][c] = TrackDeepestPoint[indexus]->data[r][c];
                        TrackArea[index]->data[r][c] = TrackArea[indexus]->data[r][c];
                        TrackFoS[index]->data[r][c] = TrackFoS[indexus]->data[r][c];
                    }


                    if(indexus > -1 && indexus < max_index)
                    {
                        TrackElevation[indexus]->data[r][c] = h;
                        TrackVolume[indexus]->data[r][c] = volume;
                        TrackVolumeIntersect[indexus]->data[r][c] = (4.0/3.0) * 3.14 * size * size_lat * size_vert;
                        TrackDeepestPoint[indexus]->data[r][c] = z_min;
                        TrackArea[indexus]->data[r][c] = area;
                        TrackFoS[indexus]->data[r][c] = FoS;
                    }
    ...
    return {{TrackIndices,TrackIndicesUS,SF,Depth,Probability,ellipsoid_x,ellipsoid_y,ellipsoid_z,ellipsoid_a,ellipsoid_b,ellipsoid_c,ellipsoid_t1,ellipsoid_t2},TrackArea,TrackVolume,TrackVolumeIntersect,TrackElevation,TrackFoS,TrackDeepestPoint};
