...
inline cTMap * AS_FlowSuperFast(cTMap * DEM,  cTMap * N,cTMap * Rain, float duration, bool do_depressions, bool corr, int itersp = -1)
{
    ...
    float dx = DEM->cellSizeX();
    int iters= 75 * std::max(1,std::max(DEM->nrCols(),DEM->nrRows())/1000);
    cTMap * GradX1 = AS_SlopeX1(DEM);
    cTMap * GradY1 = AS_SlopeY1(DEM);
    cTMap * GradX2 = AS_SlopeX2(DEM);
    cTMap * GradY2 = AS_SlopeY2(DEM);
    ...
    {
        DEM2 = AS_SpreadMonotonicReconstruct(DEM,0.001,iters);//AS_SpreadDirectionalAbsMaxMD(Seeds,DEM,GradX1,GradX2,GradY1,GradY2);
...
//super-fast flood algorithm with multiple pressure sub-iterations
//besides some multi-threading upportunities, SSE/AVX instructions, etc
//from a data-dependency perspective I dont see how any physically-based method could be significantly faster than this
//the number of iterations in space are reduced by carefully selected method so that there is only minimal number of iterations and complexity
inline cTMap * AS_FlowSuperFastPI(cTMap * DEM_ORG,  cTMap * N,cTMap * Rain_ORG, float duration, int piters, bool do_depressions, bool corr, int itersp = -1)
{
    ...
    std::cout << "super fast flow pi : " << piters << std::endl;
    for(int ip = 0; ip < piters; ip++)
    {
        ...
        std::cout << "pressure iteration " << ip << std::endl;
            ...
            for (c = 0; c < DEM->Cols(); c++) {
                if (!pcr::isMV(DEM->data[r][c])) {
                    ...
                    if(!do_depressions)
                    {
                        ...
                        float h_old = std::pow((1.0/(dx)) * ss_comp * q_old * RainN->data[r][c]/std::max(0.001f,Slope),3.0/5.0);
                        one->data[r][c] = std::max(0.0,-h_old + std::pow((1.0/(dx)) * ss_comp * (q->data[r][c] + q_old) * RainN->data[r][c]/std::max(0.001f,Slope),3.0/5.0));
                    ...
                    }else
                    {
                        float q_old = QSS_PREV->data[r][c];
                        float h_old = std::pow((1.0/(dx)) * ss_comp * q_old * RainN->data[r][c]/std::max(0.001f,Slope),3.0/5.0);
                        one->data[r][c] = std::max(0.0,-h_old + std::pow((1.0/(dx)) *ss_comp * (accqreal->data[r][c] + q_old) * RainN->data[r][c]/std::max(0.001f,Slope),3.0/5.0));
                        WH_PREV->data[r][c] += one->data[r][c];
