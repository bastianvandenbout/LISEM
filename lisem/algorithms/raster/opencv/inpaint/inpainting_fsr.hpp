...
#define INPAINT_FSR_FAST 0
#define INPAINT_FSR_BEST 1
...
struct fsr_parameters
{
    ...
    fsr_parameters(const int quality, int iterations,int ffsize, int iss,double rhoi, int bs)
    {
        block_size = bs;
        for(int i = 0; i < 4; i++)
        {
        ...
        intrinsicsamplingsize = iss;
        if (quality == INPAINT_FSR_BEST)
        {
            block_size_min = 2;
            fft_size = 128;
            max_iter = 1600;
        ...
        else if (quality == INPAINT_FSR_FAST)
        {
            block_size_min = 4;
            fft_size = 64;
            max_iter = 400;
...
icvStandardDeviation(const Mat& distorted_block_2d, const Mat& error_mask_2d) {
    ...
    double sigma_n = tmp_stddev[0]/255.0;
    if (sigma_n < 0)
    {
        sigma_n = 0;
    }
...
static inline double ext_wf(double period, double period_block, double block_dist, double block_dist2)
{
    return std::fabs(period -period_block) < block_dist? (0.0):(std::min(1.0,(std::fabs(period -period_block)-block_dist)/ block_dist2));
}


static void
icvExtrapolateBlock(Mat& distorted_block, Mat& error_mask, fsr_parameters& fsr_params, double rho, double normedStdDev, Mat& extrapolated_block,Mat& extrapolated_block2, Mat& spectrum)
{
    double fft_size = fsr_params.fft_size;
    double orthogonality_correction = fsr_params.orthogonality_correction;
    int intrinsicsamplingsize = fsr_params.intrinsicsamplingsize;
    ...
    for (int u = 0; u < fft_size; ++u)
    {
        ...
        {
            w.at<double>(u, v) *= std::pow(rho, std::sqrt(std::pow(u + 0.5 - (fft_y_offset + M / 2.0), 2.0) + std::pow(v + 0.5 - (fft_x_offset + N / 2.0), 2.0)));
        }
    }
    ...
    for (int y = 0; y < fft_size; ++y)
    {
        ...
        {
            double y2 = fft_size / 2.0 - std::abs(y - fft_size / 2.0);
            double x2 = fft_size / 2.0 - std::abs(x - fft_size / 2.0);
            if(intrinsicsamplingsize > 0)
            {
                double periodx = fft_size/((double)(std::max(1,x)));
                double periody = fft_size/((double)(std::max(1,y)));

                double periodxi = fft_size/((double)(fft_size - x));
                double periodyi = fft_size/((double)(fft_size - y));

                float fac_intrinsic_x = (periodx > ((double)(intrinsicsamplingsize - 1.0)) && periodx < ((double)(intrinsicsamplingsize + 1.75))) ? 0.0:1.0;// 1.0-(std::exp(-0.05*std::pow((periodx - ((double)(intrinsicsamplingsize))),6.0)));
                float fac_intrinsic_y = (periody > ((double)(intrinsicsamplingsize - 1.0)) && periody < ((double)(intrinsicsamplingsize + 1.75))) ? 0.0:1.0;//1.0-(std::exp(-0.05*std::pow((periody - ((double)(intrinsicsamplingsize))),6.0)));

                fac_intrinsic_x = fac_intrinsic_x*((periodx > ((double)(intrinsicsamplingsize/2 - 0.25)) && periodx < ((double)(intrinsicsamplingsize/2 + 0.5))) ? 0.0:1.0);// 1.0-(std::exp(-0.05*std::pow((periodx - ((double)(intrinsicsamplingsize))),6.0)));
                fac_intrinsic_y = fac_intrinsic_y*((periody > ((double)(intrinsicsamplingsize/2 - 0.25)) && periody < ((double)(intrinsicsamplingsize/2 + 0.5))) ? 0.0:1.0);//1.0-(std::exp(-0.05*std::pow((periody - ((double)(intrinsicsamplingsize))),6.0)));
                fac_intrinsic_x = fac_intrinsic_x*((periodx > ((double)(intrinsicsamplingsize/4 - 0.125)) && periodx < ((double)(intrinsicsamplingsize/4 + 0.25))) ? 0.0:1.0);// 1.0-(std::exp(-0.05*std::pow((periodx - ((double)(intrinsicsamplingsize))),6.0)));
                fac_intrinsic_y = fac_intrinsic_y*((periody > ((double)(intrinsicsamplingsize/4 - 0.125)) && periody < ((double)(intrinsicsamplingsize/4 + 0.25))) ? 0.0:1.0);//1.0-(std::exp(-0.05*std::pow((periody - ((double)(intrinsicsamplingsize))),6.0)));

                float fac_intrinsic_xi = (periodxi > ((double)(intrinsicsamplingsize - 1.0)) && periodxi < ((double)(intrinsicsamplingsize + 1.75))) ? 0.0:1.0;// 1.0-(std::exp(-0.05*std::pow((periodx - ((double)(intrinsicsamplingsize))),6.0)));
                float fac_intrinsic_yi = (periodyi > ((double)(intrinsicsamplingsize - 1.0)) && periodyi < ((double)(intrinsicsamplingsize + 1.75))) ? 0.0:1.0;//1.0-(std::exp(-0.05*std::pow((periody - ((double)(intrinsicsamplingsize))),6.0)));

                //fac_intrinsic_xi = fac_intrinsic_xi*((periodxi > ((double)(intrinsicsamplingsize/2 - 0.25)) && periodxi < ((double)(intrinsicsamplingsize/2 + 0.5))) ? 0.0:1.0);// 1.0-(std::exp(-0.05*std::pow((periodx - ((double)(intrinsicsamplingsize))),6.0)));
                //fac_intrinsic_yi = fac_intrinsic_yi*((periodyi > ((double)(intrinsicsamplingsize/2 - 0.25)) && periodyi < ((double)(intrinsicsamplingsize/2 + 0.5))) ? 0.0:1.0);//1.0-(std::exp(-0.05*std::pow((periody - ((double)(intrinsicsamplingsize))),6.0)));
                //fac_intrinsic_xi = fac_intrinsic_xi*((periodxi > ((double)(intrinsicsamplingsize/4 - 0.125)) && periodxi < ((double)(intrinsicsamplingsize/4 + 0.25))) ? 0.0:1.0);// 1.0-(std::exp(-0.05*std::pow((periodx - ((double)(intrinsicsamplingsize))),6.0)));
                //fac_intrinsic_yi = fac_intrinsic_yi*((periodyi > ((double)(intrinsicsamplingsize/4 - 0.125)) && periodyi < ((double)(intrinsicsamplingsize/4 + 0.25))) ? 0.0:1.0);//1.0-(std::exp(-0.05*std::pow((periody - ((double)(intrinsicsamplingsize))),6.0)));


                frequency_weighting.at<double>(y, x) = std::min(fac_intrinsic_x,fac_intrinsic_y) *std::min(fac_intrinsic_xi,fac_intrinsic_yi) *1.0;//(1.0 - std::sqrt(x2*x2 + y2 * y2)*std::sqrt(2.0) / fft_size);
            }else {
                frequency_weighting.at<double>(y, x) = std::max(0.0,1.0 - std::sqrt(x2*x2 + y2 * y2)*std::sqrt(2.0) / fft_size);
            }
        }
    }
    ...
    extrapolated_block.copyTo(extrapolated_block2);
...
icvGetTodoBlocks(Mat& sampled_img, Mat& sampling_mask, std::vector< std::tuple< int, int > >& set_todo, int block_size, int block_size_min, int border_width, double homo_threshold, Mat& set_process_this_block_size, std::vector< std::tuple< int, int > >& set_later, Mat& sigma_n_array)
{
    ...
    //std::cout << "get to do blocks 1 " << std::endl;
    ...
    //std::cout << "get to do blocks 2 " << std::endl;
    ...
    for (size_t entry = 0; entry < list_length; ++entry)
    {
        ...
        if (block_size > block_size_min)
        {
            ...
            //std::cout << sigma_n_array.size[0] << " " <<sigma_n_array.size[1] << " " << yblock_counter<< " "<< xblock_counter << std::endl;
    ...
    //std::cout << "get to do blocks 3 " << std::endl;
}
...
icvDetermineProcessingOrder(
    ...
    const int quality, const std::string& channel, Mat& reconstructed_img_res, int iterations, int ffsize, int iss, double rhoi, int bs, double ms
)
{
    //std::cout << 11 << std::endl;
    fsr_parameters fsr_params(quality,iterations,ffsize,iss,rhoi,bs);
    ...
    Mat sampled_img;
    _sampled_img.copyTo(sampled_img);
    Mat sampling_mask;
    _sampling_mask.copyTo(sampling_mask);
    //_sampled_img.convertTo(sampled_img, CV_64F);
    reconstructed_img_res = sampled_img.clone();
    Mat reconstructed_img = sampled_img.clone();
    Mat reconstructed_img_w = sampled_img.clone();
    reconstructed_img_res.setTo(0.0);
    reconstructed_img_w.setTo(0.0);

    Mat spect;
    bool spect_set = false;
    double nspect = 0.0;
...
//std::cout << 13 << std::endl;
    ...
    int blocks_column = cvCeil(img_height / block_size);//divUp(img_height, block_size);
    int blocks_line = cvCeil(img_width / block_size);//divUp(img_width, block_size);
    ...
    //std::cout << 15 << " " << block_size << " " << blocks_column << " " << blocks_line <<  " " << block_size_min << std::endl;
    ...
    while (block_size >= block_size_min)
    {
        ...
        if (block_size > block_size_min)
        {
            ...
            //std::cout << "get to do blocks " << std::endl;
        ...
        //std::cout << "Block check pre" << std::endl;
        ...
        for (int yblock_counter = 0; yblock_counter < blocks_per_column; ++yblock_counter)
        {
            ...
            {
                //std::cout << "Block check pre 1 " << yblock_counter << " " << xblock_counter << std::endl;
                ...
                //std::cout << "Block check pre 2 " << yblock_counter*block_size << " " << std::min(img_height, (yblock_counter + 1)*block_size) << " " << xblock_counter*block_size << "  " << std::min(img_width, (xblock_counter + 1)*block_size) <<  std::endl;
                double min_block, max_block;
                minMaxLoc(curr_block, &min_block, &max_block);
                if (min_block == 0)
                {
        ...
        //std::cout << "start check blocks" << std::endl;
        ...
        for (int yblock_counter = 0; yblock_counter < blocks_per_column; ++yblock_counter)
        {
            for (int xblock_counter = 0; xblock_counter < blocks_per_line; ++xblock_counter)
            {
                //std::cout << "check block 1" <<std::endl;
                Mat curr_block = sampling_mask(Range(yblock_counter*block_size, std::min(img_height, (yblock_counter + 1)*block_size)), Range(xblock_counter*block_size, std::min(img_width, (xblock_counter + 1)*block_size)));
                //std::cout << "check block 2" <<std::endl;
                double min_block, max_block;
                minMaxLoc(curr_block, &min_block, &max_block);
                if (min_block != 0)
                {
        ...
        while (all_blocks_finished == 0) {
            ...
            // decision if block contains errors
                    ...
                    if (tmp_val == min_nen && proc_array.at<double>(yblock_counter, xblock_counter) != 0 && set_process_this_block_size.at<double>(yblock_counter, xblock_counter) != 0) {
                        ...
                        //do processing
                        //std::cout << "add to processing list " << min_nen << block_list.size() << std::endl;
            ...
            //std::cout << "blocks to do " << max_bl_counter << std::endl;
            ...
            for (bl_counter = 0; bl_counter < max_bl_counter; ++bl_counter)
            {
                ...
                Mat extrapolated_block_2d2;
                Mat spec;
                icvExtrapolateBlock(distorted_block_2d, error_mask_2d, fsr_params, rho, sigma_n_a, extrapolated_block_2d,extrapolated_block_2d2, spec);
                ...
                //update moving blended image

                for(int c = 0; c < extrapolated_block_2d.cols; c++)
                {
                    for(int r = 0; r < extrapolated_block_2d.rows; r++)
                    {
                        int r2 = yblock_counter*block_size + r - top_border;
                        int c2 = xblock_counter*block_size + c - left_border;

                        if(c2 > 0 && r2 > 0 && c2 < reconstructed_img_res.cols && r2 < reconstructed_img_res.rows)
                        {
                            //get distance from square
                            int distc = 0;
                            int distr = 0;
                            if(c < top_border )
                            {
                            ...
                            }else if(c > top_border + block_size)
                            {
                                distc = c - (top_border+block_size);
                            }
                            if(r < left_border)
                            {
                                distr = left_border - r;

                            }else if(r > left_border + block_size)
                            {
                                distr = r - (left_border +block_size);
                            }

                            double dist = 0;
                            if(distc > 0 && distr > 0)
                            {
                                dist = std::sqrt((double)(distc*distc + distr*distr));

                            }else {
                                dist = std::max(distc,distr);
                            }

                            //set weight and value

                            double weight = std::max(0.0,1.0 - dist/(ms*((double)(block_size))));
                            if(!(ms > 0.0))
                            {
                                weight = dist > 0? 0.0:1.0;
                            ...
                            if(weight > 0.0)
                            {
                                ...
                                weight = 6.0 * std::pow(weight,5.0) - 15.0 * std::pow(weight,4.0) + 10.0 * std::pow(weight,3.0);

                                reconstructed_img_res.at<double>(r2,c2) += weight * extrapolated_block_2d2.at<double>(r,c);
                                reconstructed_img_w.at<double>(r2,c2) += weight;
                            }
                        }
                ...
                //extrapolated_block_2d2(Range(top_border, extrapolated_block_2d.rows - bottom_border), Range(left_border, extrapolated_block_2d.cols - right_border)).copyTo(reconstructed_img_res(Range(yblock_counter*block_size, std::min(img_height, (yblock_counter + 1)*block_size)), Range(xblock_counter*block_size, std::min(img_width, (xblock_counter + 1)*block_size))));
        ...
        std::cout << "process " << block_size << std::endl;
    ...
    for(int c = 0; c < reconstructed_img_res.cols; c++)
    {
        for(int r = 0; r < reconstructed_img_res.rows; r++)
        {

            double weight = reconstructed_img_w.at<double>(r,c);
            if(weight > 0.0)
            {
                reconstructed_img_res.at<double>(r,c) = reconstructed_img_res.at<double>(r,c)/weight;
            }
        }
    }
    ...
    {
        for (int y = 0; y < spect.rows; ++y)
        {

            for(int x = 0; x < spect.cols; ++x)
            {

                std::cout << spect.at<std::complex<double>>(y,x).real()/nspect << " ";
...
static
void inpaint_fsr(Mat &src, Mat &mask, Mat &dst, const int algorithmType, int iterations, int ffsize,int iss, double rho = -1.0, int bs=16, double ms=1.0)
{
    CV_Assert(algorithmType == INPAINT_FSR_BEST || algorithmType == INPAINT_FSR_FAST);
    ...
    { // grayscale image
        Mat y_reconstructed;
        icvDetermineProcessingOrder(src, mask_01, algorithmType, "Y", y_reconstructed,iterations,ffsize,iss, rho,bs,ms);
        dst = y_reconstructed.clone();
    }
    ...
    { // RGB image
        ...
        cb = cb.mul(mask_01);
        cr = cr.mul(mask_01);
        icvDetermineProcessingOrder(y, mask_01, algorithmType, "Y", y_reconstructed, iterations,ffsize,iss,rho,bs,ms);
        icvDetermineProcessingOrder(cb, mask_01, algorithmType, "Cx", cb_reconstructed, iterations,ffsize,iss,rho,bs,ms);
        icvDetermineProcessingOrder(cr, mask_01, algorithmType, "Cx", cr_reconstructed, iterations,ffsize,iss,rho,bs,ms);
