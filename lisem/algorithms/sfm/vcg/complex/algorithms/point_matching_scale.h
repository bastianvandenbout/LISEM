...
#include <sfm/wrap/newuoa/include/newuoa.h>
...
private:
	static std::vector<sfm/vcg::Point3d> *fix;
	static std::vector<sfm/vcg::Point3d> *mov;
	static vcg::Box3d b;
...
public:
	...
	static void computeScalingMatchMatrix(
			vcg::Matrix44d &res,
			std::vector<sfm/vcg::Point3d> &Pfix,
			std::vector<sfm/vcg::Point3d> &Pmov)
	{
		fix = &Pfix;
		mov = &Pmov;
		b.SetNull();
		for(std::vector<sfm/vcg::Point3d>::iterator i = Pmov.begin(); i != Pmov.end(); ++i)
			b.Add(*i);
	...
	static double errorScale(int n, double *x)
	{
		assert(n==1); (void)n;
		double dist = 0;
		std::vector<sfm/vcg::Point3d>::iterator i = mov->begin();
		std::vector<sfm/vcg::Point3d>::iterator ifix = fix->begin();
