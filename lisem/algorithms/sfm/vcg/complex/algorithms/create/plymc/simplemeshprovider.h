...
#include <sfm/wrap/io_trimesh/alnParser.h>
#include <list>
#include <vector>
...
private:
  ...
  std::vector<sfm/vcg::Matrix44f> trVec;
...
private:
  ...
  std::vector<sfm/vcg::Matrix44f> TrV;
  std::vector<float> WV;		        // weight tot be applied to each mesh.
  std::vector<sfm/vcg::Box3f> BBV;	    // bbox of the transformed meshes..
  vcg::Box3f fullBBox;
