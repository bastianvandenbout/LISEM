...
#include <vector>
#include <ctime>
#include <sfm/vcg/space/box2.h>
#include <sfm/vcg/space/point2.h>
#include <sfm/vcg/math/similarity2.h>
...
//    printf("areaSum %6.3f\n",areaSum);
    std::vector<sfm/vcg::Point2i> sizes(rectVec.size());
    for (size_t i=0;i<rectVec.size();++i)
    {
    ...
    std::vector<sfm/vcg::Point2i> posiz;
...
//    printf("areaSum %6.3f\n",areaSum);
    ...
    std::vector<sfm/vcg::Point2i> posiz;
    std::vector<sfm/vcg::Point2i> global_sizeVec;
...
*/
static bool PackInt(const std::vector<sfm/vcg::Point2i> & sizes, // the sizes of the rect to be packed
	const vcg::Point2i & max_size,           // the size of the container
	std::vector<sfm/vcg::Point2i> & posiz,       // the found positionsof each rect
...
{
public:
  const std::vector<sfm/vcg::Point2i> & v;
  inline ComparisonFunctor( const std::vector<sfm/vcg::Point2i> & nv ) : v(nv) { }
