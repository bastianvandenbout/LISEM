    ...
    for (int i = (checkPartialSubsets?count-1:2); i < count; ++i) {
        ...
        for (int j = 1; j < i; ++j) {
            ...
            for (int k = 0; k < j; ++k) {
                ...
                if (MVSFABS(d2.x*d1.y - d2.y*d1.x) <= FLT_EPSILON*(MVSFABS(d1.x) + MVSFABS(d1.y) + MVSFABS(d2.x) + MVSFABS(d2.y)))
                    return true;
            }
        }
    }
...
inline TYPE FindAbsMinElement(const TYPE* values, size_t n) {
    TYPE m = std::numeric_limits<TYPE>::max();
    while (n) if (MVSFABS(values[--n]) < m) m = MVSFABS(values[n]);
    return m;
}
template<typename TYPE>
...
inline std::pair<TYPEW,TYPEW> ComputeX84Threshold(const TYPE* const values, size_t size, TYPEW mul=TYPEW(5.2), const uint8_t* mask=NULL) {
    ...
    // threshold = 5.2 * MEDIAN(MVSFABS(values-median));
    using TYPEI = typename MakeSigned<TYPE>::type;
    for (TYPE& val: data)
        val = TYPE(MVSFABS(TYPEI(val)-TYPEI(median)));
    std::nth_element(data.Begin(), mid, data.End());
...
void KernelDensityEstimation(const TMatrix<TYPE,2,1>* samples, size_t numSamples, TYPE bandwidth, TYPE xMin, TYPE xMax, TYPE stepMax, INSERTER& inserter) {
    ...
    do {
        ...
        if (smallSteps) {
            ...
            if (MVSFABS(SIGN(last_delta)+SIGN(delta)) < 2 && inserter.SamplePeak(last_delta>0)) {
                ...
                last_x = x;
                x -= (MVSFABS(delta) < MVSFABS(last_delta) ? step : step*TYPE(2));
                smallSteps = 5;
...
struct CBlakeZisserman { // Blake-Zisserman Gaussian + uniform
    ...
    inline TYPE operator()(const TYPE d) const {
        ...
        const TYPE d_abs = MVSFABS(d) + 1e-12;
...
struct CL1 {
    inline TYPE operator()(const TYPE d) const {
        return TYPE(1) / (SQRT(MVSFABS(d)) + 1e-12);
    }
};
