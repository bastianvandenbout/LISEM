...
TYPE TQuaternion<TYPE>::GetRotationAngle() const
{
    ...
    return std::isnan(angle) && MVSFABS(MVSFABS((*this)[3]) - TYPE(1)) < QUATERNION_EPSILON ?
        TYPE(0) :
        angle;
}
...
void TRMatrixBase<TYPE>::EnforceOrthogonality()
{
    ...
    *this = svd.u*svd.vt;
    ...
    for (unsigned iterate = 0; iterate != 10; iterate++) {
        ...
        //Check how off it is.
        x[0] = MVSFABS(r[0]*r[0] + r[3]*r[3] + r[6]*r[6] - TYPE(1));
        x[1] = MVSFABS(r[1]*r[1] + r[4]*r[4] + r[7]*r[7] - TYPE(1));
        x[2] = MVSFABS(r[2]*r[2] + r[5]*r[5] + r[8]*r[8] - TYPE(1));
        x[3] = MVSFABS(r[0]*r[1] + r[3]*r[4] + r[6]*r[7]);
        x[4] = MVSFABS(r[1]*r[2] + r[4]*r[5] + r[7]*r[8]);
        x[5] = MVSFABS(r[2]*r[0] + r[5]*r[3] + r[8]*r[6]);
