...
inline GCLASS TPlane<TYPE,DIMS>::Classify(const POINT& p) const
{
    ...
    if (f >  ZEROTOLERANCE<TYPE,DIMS>()) return MVSFRONT;
    if (f < -ZEROTOLERANCE<TYPE,DIMS>()) return MVSBACK;
	return PLANAR;
}
/*----------------------------------------------------------------*/
...
GCLASS TFrustum<TYPE,DIMS>::Classify(const POINT& p) const
{
	...
	for (int i=0; i<DIMS; ++i) {
        if (m_planes[i].Classify(p) == MVSFRONT)
			return CULLED;
	} // for
	return VISIBLE;
}
