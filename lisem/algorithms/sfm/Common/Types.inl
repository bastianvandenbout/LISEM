...
#define MVSFABS(x) ((x) <0.0? -(x) : (x))

// abs
template <typename TYPE>
FORCEINLINE SEACAVE::TPoint2<TYPE> MVSABS(const SEACAVE::TPoint2<TYPE>& v)
{
    return SEACAVE::TPoint2<TYPE>(MVSFABS(v.x), MVSFABS(v.y));
}
template <typename TYPE>
FORCEINLINE SEACAVE::TPoint3<TYPE> MVSABS(const SEACAVE::TPoint3<TYPE>& v)
{
    return SEACAVE::TPoint3<TYPE>(MVSFABS(v.x), MVSFABS(v.y), MVSFABS(v.z));
}
template <typename TYPE, int m, int n>
FORCEINLINE SEACAVE::TMatrix<TYPE,m,n> MVSABS(const SEACAVE::TMatrix<TYPE,m,n>& v)
{
	SEACAVE::TMatrix<TYPE,m,n> nv;
	for (int i=0; i<m*n; ++i)
        nv(i) = MVSFABS(v.val[i]);
	return nv;
}
template <typename TYPE>
FORCEINLINE SEACAVE::TPoint2<TYPE> MVSABS(const cv::Point_<TYPE>& v)
{
    return SEACAVE::TPoint2<TYPE>(MVSFABS(v.x), MVSFABS(v.y));
}
template <typename TYPE>
FORCEINLINE SEACAVE::TPoint3<TYPE> MVSABS(const cv::Point3_<TYPE>& v)
{
    return SEACAVE::TPoint3<TYPE>(MVSFABS(v.x), MVSFABS(v.y), MVSFABS(v.z));
}
template <typename TYPE, int m, int n>
FORCEINLINE SEACAVE::TMatrix<TYPE,m,n> MVSABS(const cv::Matx<TYPE,m,n>& v)
{
	SEACAVE::TMatrix<TYPE,m,n> nv;
	for (int i=0; i<m*n; ++i)
        nv(i) = MVSFABS(v.val[i]);
...
inline TYPE area(const TPoint2<TYPE>& v1, const TPoint2<TYPE>& v2) {
    return MVSFABS(v1.x * v2.y - v1.y * v2.x);
}
template <typename TYPE>
...
inline bool equal<double>(const double& left, const double& right, const double& eps) {
	...
	if (left==0. || right==0.)
        return (MVSFABS(left-right) <= eps);
	// compare two non-zero values
    return (std::abs(left-right) <= eps*MAXF(std::abs(right),std::abs(left)));
}
template<>
...
void TImage<TYPE>::DrawLine(const TPoint2<T>& p1, const TPoint2<T>& p2, PARSER& parser)
{
    ...
    const bool steep(MVSFABS(y2 - y1) > MVSFABS(x2 - x1));
	if (steep) {
		std::swap(x1, y1);
		std::swap(x2, y2);
	}
