...
IMAGEPTR Image::ReadImageHeader(const String& fileName)
{
	...
	if (pImage == NULL || FAILED(pImage->ReadHeader())) {
        LISEMS_ERROR("error: failed loading image header, file is " + QString(pImage == NULL? "null":"found"));
		pImage.Release();
	}
	return pImage;
} // ReadImageHeader
...
bool Image::LoadImage(const String& fileName, unsigned nMaxResolution)
{
	...
	IMAGEPTR pImage(OpenImage(fileName));
	if (pImage == NULL) {
        LISEMS_ERROR("error: failed opening input image " + QString(name.c_str()));
		return false;
	}
	// create and fill image data
	if (!ReadImage(pImage, image)) {
        LISEMS_ERROR("error: failed loading image " + QString(name.c_str()));
		return false;
	}
...
bool Image::ReloadImage(unsigned nMaxResolution, bool bLoadPixels)
{
	...
	if (pImage == NULL) {
        LISEMS_ERROR("error: failed reloading image '%s'" + QString(name.c_str()));
		return false;
	}
	if (!bLoadPixels) {
		// init image size
