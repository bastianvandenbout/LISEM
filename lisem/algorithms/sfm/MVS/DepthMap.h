...
__declspec(dllexport) struct DepthEstimator {
...
public:
	...
	struct ViewData {
		...
		const Matrix3x3 Hl;   //
        const TMatrix<REAL,3,1> Hm;	      // constants during per-pixel loops
		const Matrix3x3 Hr;   //
    ...
    inline static void ComputeRelativeRotation(const TMatrix<TYPE,3,3>& Ri, const TMatrix<TYPE,3,3>& Rj, TMatrix<TYPE,3,3>& Rij) {
        Rij = Rj * Ri.t();
    } // ComputeRelativeRotation
    template<typename TYPE>
    inline static void ComputeRelativePose(const TMatrix<TYPE,3,3>& Ri, const TPoint3<TYPE>& Ci, const TMatrix<TYPE,3,3>& Rj, const TPoint3<TYPE>& Cj, TMatrix<TYPE,3,3>& Rij, TPoint3<TYPE>& Cij) {
        Rij = Rj * Ri.t();
        Cij = Ri * (Cj - Ci);
    } // ComputeRelativePose
    /*----------------------------------------------------------------*/
    ...
    template <typename TYPE>
    inline static TMatrix<TYPE,3,3> HomographyMatrixComposition(const TMatrix<TYPE,3,3>& R, const TPoint3<TYPE>& C, const TMatrix<TYPE,3,1>& n, const TYPE& d) {
        const TMatrix<TYPE,3,1> t(R*(-C));
        return TMatrix<TYPE,3,3>(R - t*(n.t()*INVERT(d)));
    }
    template <typename TYPE>
    inline static TMatrix<TYPE,3,3> HomographyMatrixComposition(const TMatrix<TYPE,3,3>& R, const TPoint3<TYPE>& C, const TMatrix<TYPE,3,1>& n, const TMatrix<TYPE,3,1>& X) {
        return HomographyMatrixComposition(R, C, n, -n.dot(X));
    }
    template <typename TYPE>
    inline static TMatrix<TYPE,3,3> HomographyMatrixComposition(const TMatrix<TYPE,3,3>& Ri, const TPoint3<TYPE>& Ci, const TMatrix<TYPE,3,3>& Rj, const TPoint3<TYPE>& Cj, const TMatrix<TYPE,3,1>& n, const TYPE& d) {
        #if 0
    ...
    template <typename TYPE>
    inline static TMatrix<TYPE,3,3> HomographyMatrixComposition(const TMatrix<TYPE,3,3>& Ri, const TPoint3<TYPE>& Ci, const TMatrix<TYPE,3,3>& Rj, const TPoint3<TYPE>& Cj, const TMatrix<TYPE,3,1>& n, const TMatrix<TYPE,3,1>& X) {
        #if 0
        Matrix3x3 R; Point3 C;
        ComputeRelativePose(Ri, Ci, Rj, Cj, R, C);
	...
	inline Matrix3x3f ComputeHomographyMatrix(const ViewData& img, Depth depth, const Normal& normal) const {
        ...
        const Matrix3x3f H(img.view.camera.K*HomographyMatrixComposition(image0.camera.C,  TMatrix<REAL,3,1>(normal),  img.view.camera, TMatrix<REAL,3,1>(X0*depth))*image0.camera.K.inv());
        return H;
        #else
		// compute homography matrix as above, caching some constants
        const TMatrix<REAL,3,1> n(normal);
        Matrix3x3d m = (img.Hl + img.Hm * (n.t()*INVERT(n.dot(X0)*depth))) * img.Hr;
        Matrix3x3f m2 = m;
        return m2;
        #endif
	}
