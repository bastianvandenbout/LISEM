...
{
    std::cout << "init depth map "<<std::endl;
	TD_TIMER_STARTD();
...
void* STCALL DepthMapsData::EstimateDepthMapTmp(void* arg)
{
    ...
    std::cout << "!!!!!  do estimate " << estimator.images.size() << std::endl;
    if(estimator.images.size() > 0)
    {
        ExportConfidenceMap(ComposeDepthFilePath(1, "image0.png"), estimator.images[0].view.image);
    }
    if(estimator.images.size() > 1)
    {
        ExportConfidenceMap(ComposeDepthFilePath(1, "image1.png"), estimator.images[1].view.image);
    }
    if(estimator.images.size() > 2)
    {
        ExportConfidenceMap(ComposeDepthFilePath(1, "image2.png"), estimator.images[2].view.image);
    }
    if(estimator.images.size() > 3)
    {
        ExportConfidenceMap(ComposeDepthFilePath(1, "image3.png"), estimator.images[3].view.image);
    }
...
{
    std::cout << "do random refinement "<<std::endl;
	TD_TIMER_STARTD();
	...
	// initialize the reference confidence map (NCC score map) with the score of the current estimates
	{
        ...
        //if (g_nVerbosityLevel > 4) {
            ...
            ExportDepthMap(ComposeDepthFilePath(image.GetID(), "conf0.png"), depthData.confMap);
...
{
    std::cout << "compute depth maps "<< std::endl;
	{
	...
	// prepare images for dense reconstruction (load if needed)
	{
		...
		FOREACH(idxImage, images) {
            ...
            if (!imageData.ReloadImage(nMaxResolution)) {
				#ifdef DENSE_USE_OPENMP
                LISEMS_ERROR("Could not reload image at resolution " + QString::number(nMaxResolution) + " " + QString::number(OPTDENSE::nResolutionLevel) + " " + imageData.name);
                bAbort = true;
				#pragma omp flush (bAbort)
				continue;
				#else
		...
		#endif
            LISEMS_ERROR("error: preparing images for dense reconstruction failed (errors loading images)");
			return false;
		}
        LISEMS_STATUS("Preparing images for dense reconstruction completed " + QString::number(images.GetSize()));
    }
	...
	// select images to be used for dense reconstruction
	{
		...
		if (OPTDENSE::nNumViews == 1 && !data.depthMaps.SelectViews(data.images, imagesMap, data.neighborsMap)) {
            LISEMS_ERROR("error: no valid images to be dense reconstructed");
			return false;
		}
		ASSERT(!data.images.IsEmpty());
        LISEMS_STATUS("Selecting images for dense reconstruction completed: " + QString::number(data.images.GetSize()));
	}
	}
