...
/*namespace OPTDENSE
{
MVS_API  unsigned nResolutionLevel;
MVS_API  unsigned nMaxResolution;
MVS_API  unsigned nMinResolution;
MVS_API  unsigned nMinViews;
MVS_API  unsigned nMaxViews;
MVS_API  unsigned nMinViewsFuse;
MVS_API  unsigned nMinViewsFilter;
MVS_API  unsigned nMinViewsFilterAdjust;
MVS_API  unsigned nMinViewsTrustPoint;
MVS_API  unsigned nNumViews;
MVS_API  bool bFilterAdjust;
MVS_API  bool bAddCorners;
MVS_API  float fViewMinScore;
MVS_API  float fViewMinScoreRatio;
MVS_API  float fMinArea;
MVS_API  float fMinAngle;
MVS_API  float fOptimAngle;
MVS_API  float fMaxAngle;
MVS_API  float fDescriptorMinMagnitudeThreshold;
MVS_API  float fDepthDiffThreshold;
MVS_API  float fNormalDiffThreshold;
MVS_API  float fPairwiseMul;
MVS_API  float fOptimizerEps;
MVS_API  int nOptimizerMaxIters;
MVS_API  unsigned nSpeckleSize;
MVS_API  unsigned nIpolGapSize;
MVS_API  unsigned nOptimize;
MVS_API  unsigned nEstimateColors;
MVS_API  unsigned nEstimateNormals;
MVS_API  float fNCCThresholdKeep;
MVS_API  unsigned nEstimationIters;
MVS_API  unsigned nRandomIters;
MVS_API  unsigned nRandomMaxScale;
MVS_API  float fRandomDepthRatio;
MVS_API  float fRandomAngle1Range;
MVS_API  float fRandomAngle2Range;
MVS_API  float fRandomSmoothDepth;
MVS_API  float fRandomSmoothNormal;
MVS_API  float fRandomSmoothBonus;
}*/
...
DepthEstimator::DepthEstimator(
	...
	#endif
{
    std::cout << "!!!!!!! OPTDENSE STUFF " << OPTDENSE::fRandomSmoothBonus << " " << OPTDENSE::fRandomSmoothDepth << " " << OPTDENSE::fRandomAngle1Range <<" " <<  OPTDENSE::fNCCThresholdKeep << std::endl;
...
bool DepthEstimator::FillPixelPatch()
{
    ...
    //std::cout << "nTex " << nTexels <<  std::endl;
    ...
    for (int i=-nSizeHalfWindow; i<=nSizeHalfWindow; i+=nSizeStep)
    {
        ...
        {
            mean2 += image0.image(x0.y+i, x0.x+j);
            nmean2 ++;
        }
    }
    mean2 = mean2 / ((double)(nmean2));
	for (int i=-nSizeHalfWindow; i<=nSizeHalfWindow; i+=nSizeStep)
    {
		for (int j=-nSizeHalfWindow; j<=nSizeHalfWindow; j+=nSizeStep)
        {
            //std::cout << "pixel  " << x0.y+i << " " <<  x0.x+j << " " <<image0.image(x0.y+i, x0.x+j) << "  " << mean << " " << mean2 << " " << nmean2 << " " <<  std::endl;
            *pTexel0 = image0.image(x0.y+i, x0.x+j)-mean2;
            normSq0 += SQUARE(*pTexel0);
            pTexel0++;
        }
    }
	#else
	...
	for (int i=-nSizeHalfWindow; i<=nSizeHalfWindow; i+=nSizeStep) {
		for (int j=-nSizeHalfWindow; j<=nSizeHalfWindow; j+=nSizeStep) {
			...
			if (!image1.view.image.isInsideWithBorder<float,1>(pt))
				return thRobust;
            float testval =image1.view.image((int)pt.y, (int)pt.x);
            ...
            //std::cout << " x0 " << x0.x << " " << x0.y << " sample " << X.x << " " << X.y << " " << X.z << "     " <<  texels0(n-1) << "  " << v <<  " " << testval <<  std::endl;
...
float DepthEstimator::ScorePixel(Depth depth, const Normal& normal)
{
	...
	#if 1
    float score =scores.mean();
    //std::cout << "mean score " << score <<std::endl;
    return score;
	#else
...
void DepthEstimator::ProcessPixel(IDX idx)
{
    ...
    /*int rows = images[0].view.image.rows;
    int cols = images[0].view.image.cols;
    ...
    for(int r = 0;r < rows; r++)
    {
        for(int c = 0; c < cols; c++)
        {
            FOREACH(idxView, images)
            {
                std::cout << "value " << idxView << " " << images[idxView].view.image(Point2f(float(c),float(r))) << " " << r << " " << c << std::endl;
            }
        }
    }*/
    ...
    while(refine)
    {
        ...
        else if (conf >= thConfRand)
        {
            ...
            for (unsigned iter=0; iter<OPTDENSE::nRandomIters; ++iter) {
                ...
                ASSERT(nconf >= 0);
                //std::cout << "iter depth "<< iter << " " << OPTDENSE::nRandomIters << "conf " << conf << " depthmin/max " <<  dMinSqr << " " << dMaxSqr << " ndepth " << ndepth << " score:" << nconf << "  norm:" << nnormal.x << " " << nnormal.y << " " << nnormal.z << std::endl;
                if (conf > nconf) {
                    conf = nconf;
                    depth = ndepth;
                    normal = nnormal;
                    if (conf < thConfRand)
                    {
                        //std::cout << "found better than condition " << thConfRand << std::endl;
                        refine = true;
                        break;
                    }
                }
...
bool MVS::ExportConfidenceMap(const String& fileName, const ConfidenceMap& confMap)
{
    ...
    std::cout << "export conf " << fileName << " " << minConf << " " <<maxConf << std::endl;
...
void MVS::CompareDepthMaps(const DepthMap& depthMap, const DepthMap& depthMapGT, uint32_t idxImage, float threshold)
{
	...
	for (uint32_t i=0; i<height; ++i) {
		for (uint32_t j=0; j<width; ++j) {
            ...
            const float error(depthGT==0 ? 0 : MVSFABS(depth-depthGT)/depthGT);
			errors.Insert(error);
		}
	}
	...
	for (uint32_t i=0; i<height; ++i) {
		for (uint32_t j=0; j<width; ++j) {
            ...
            const uint8_t gray((uint8_t)CLAMP((1.f-SAFEDIVIDE(MVSFABS(error), threshold))*255.f, 0.f, 255.f));
