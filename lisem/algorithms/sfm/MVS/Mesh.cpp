...
#include "sfm/vcg/complex/complex.h"
#include "sfm/vcg/complex/algorithms/create/platonic.h"
#include "sfm/vcg/complex/algorithms/stat.h"
#include "sfm/vcg/complex/algorithms/clean.h"
#include "sfm/vcg/complex/algorithms/smooth.h"
#include "sfm/vcg/complex/algorithms/hole.h"
#include "sfm/vcg/complex/algorithms/polygon_support.h"
// VCG: mesh simplification
#include "sfm/vcg/complex/algorithms/update/position.h"
#include "sfm/vcg/complex/algorithms/update/bounding.h"
#include "sfm/vcg/complex/algorithms/update/selection.h"
#include "sfm/vcg/complex/algorithms/local_optimization.h"
#include "sfm/vcg/complex/algorithms/local_optimization/tri_edge_collapse_quadric.h"
#ifdef _MSC_VER
#  pragma warning(pop)
#endif
	...
	while ((edge.min<epsilonMin || edge.max>epsilonMax) && (no_ops>0) && (++iters<max_iters)) {
		...
		for (EdgeScoreArr::IDX i=0; i<bigEdges.GetSize(); ++i) {
			...
			if (!CanSplitEdge(h))
				continue;
			#if CURVATURE_TH>0
            const float avg_mean_curv(MAXF(ABS(h->vertex()->mean_curvature), MVSFABS(h->prev()->vertex()->mean_curvature)));
...
static void ComputeVertexCurvature(Vertex& v)
{
    ...
    float mean_curv(MVSFABS((float)v_norm(v.laplacian) / edge_avg));
...
void Mesh::ProjectOrthoTopDown(unsigned resolution, Image8U3& image, Image8U& mask, Point3& center) const
{
	...
	Camera camera;
    camera.R.SetFromDirUp(TMatrix<REAL,3,1>(Point3(0,0,-1)), TMatrix<REAL,3,1>(Point3(0,1,0)));
	camera.C = center;
	camera.C.z += size.z;
