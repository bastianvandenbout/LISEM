...
#include <sfm/vcg/space/sphere3.h>
...
public:
  ...
  void Load(std::vector<Update> &updates) {    
    ...
    typename std::vector<Update>::iterator u;
    for(u = updates.begin(); u != updates.end(); u++) {      
      ...
      typename std::map<unsigned int, std::vector<Cell *> >::iterator e;
      for(e = node_erased.begin(); e != node_erased.end(); e++) {
        ...
        typename std::vector<Cell *>::iterator k;
  ...
  template <class CONT> void Extract(CONT &selected, float error) {
    typename std::vector<Node>::iterator n;
    ...
    nodes[0].visited = true;
      ...
      typename std::vector<Frag>::iterator fragment;
      typename std::vector<Node *>::iterator on;
        ...
        if(error < (*on)->error) { //need to expand this node.
          ...
          typename std::vector<Cell *>::iterator cell;
  ...
  template <class CONT, class POLICY> void Extract(CONT &selected, POLICY &policy) {
    ...
    for( ; !qnodo.empty(); qnodo.pop()) {
      ...
      typename std::vector<Frag>::iterator i;
      typename std::vector<Node *>::iterator on;
  ...
  template <class CONT> void Extract(CONT &selected) {
    selected.clear();
    typename std::vector<Node>::iterator i;
