...
namespace io {	
	...
	public:
		...
		void generatePointsVector(std::vector<std::vector<sfm/vcg::Point3f> >& v)
		{
			for(typename PolygonalMesh::PolygonIterator itp = _pols.begin();itp != _pols.end();++itp)
			{
				v.push_back(std::vector<sfm/vcg::Point3f>());
		...
		{
			std::vector<std::vector<sfm/vcg::Point3f> > pl;
			...
			{
				std::vector<int> tx;
				std::vector<std::vector<sfm/vcg::Point3f> > pl2(1);
				pl2[0] = pl[ii];
