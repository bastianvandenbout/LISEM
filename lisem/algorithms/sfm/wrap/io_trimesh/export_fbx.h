...
private:
	...
	static bool fillScene(SaveMeshType& m,KFbxScene& scene,const int mask)
	{
		...
		std::set<sfm/vcg::Color4b> matset;	
		...
		for(std::set<sfm/vcg::Color4b>::iterator it = matset.begin();it != matset.end();++it)
		{
		...
		for(int ii = 0;ii < fn;++ii)
		{
			if (!m.face[ii].IsD())
			{
				if (wedgetexcoord)
				{
					...
					{	
						std::set<sfm/vcg::Color4b>::iterator it = matset.find(m.face[ii].V(0)->C());
