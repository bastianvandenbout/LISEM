...
#include <fstream>
#include<sfm/vcg/complex/algorithms/bitquad_support.h>
					...
					else // Standard Triangular Mesh Loading
					{
						...
						for (unsigned int f = 0; f < nFaces; f++)
						{
							...
							else
							{
								...
								std::vector<sfm/vcg::Point3f > polygonVect(vert_per_face); // vec of polygon loops used for the triangulation of polygonal face
								for (int j = 0; j < vert_per_face; j++)
								{
									if (k == tokens.size())   // if EOL // Go to next line when needed
									{
								...
								else // standard fan triangulation (we hope the polygon is convex...)
								{
									...
									vcg::glu_tesselator::tesselate<sfm/vcg::Point3f>(loopVect, indexTriangulatedVect);
#else
