...
{
    std::vector<sfm/vcg::Matrix44f> Tr(names.size());
    for(int i=0; i < static_cast<int>(Tr.size()); ++i) Tr[i].SetIdentity();
    return SaveALN(alnfile,names, Tr);
}
template <class matrixfloat>
static bool SaveALN(const char *alnfile, std::vector<std::string> &names, std::vector<sfm/vcg::Matrix44<matrixfloat> > &Tr)
{
