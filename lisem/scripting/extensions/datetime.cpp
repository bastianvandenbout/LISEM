...
void RegisterScriptDateTime(asIScriptEngine *engine)
{
    ...
    r = engine->RegisterGlobalFunction("double GetAppTime()", asFUNCTION(
                                         GetTimeInSeconds), asCALL_CDECL); assert(r >= 0);
    r = engine->RegisterGlobalFunction("void Sleep(double seconds)", asFUNCTION(
                                         AS_Sleep), asCALL_CDECL); assert(r >= 0);
