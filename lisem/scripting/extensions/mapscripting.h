...
inline void RegisterMapAlgorithmsToScriptEngine(LSMScriptEngine *engine)
{
    ...
    r = engine->RegisterGlobalSTDFunction("array<Map> @FlowIncompressible3D(array<Map> &in state, array<Map> &in Block,array<Map> &in BlockU,array<Map> &in BlockV,array<Map> &in BlockW, float csz, float dt, float visc,float courant = 0.2, float beta0 = 1.7)", GetFuncConvert(  AS_IncompressibleFlow3D));
