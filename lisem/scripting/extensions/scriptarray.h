...
public:
    ...
    void  SetValue(asUINT index, void *value, bool assign = true);
    void  SetValueNoRefAdd(asUINT index, void *value);
    ...
    template<typename A, typename std::enable_if<is_pointer<A>::value,A>::type* = nullptr>
    inline std::vector<A> ToSTDList()
    {
        std::vector<A> ret;
...
CScriptArray * AS_RangeF(double begin, double end, double step);
