...
CScriptArray * AS_RangeI(int begin, int end, int step)
{
    if( step == 0)
    {
        LISEMS_ERROR("Can not create range with step size of 0");
        throw 1;
    }
    ...
    {
        array->SetValue(i,new int(data[i]));
    }

    return array;
}
CScriptArray * AS_RangeF(float begin, float end, float step)
{
    if( step == 0.0f)
    {
        LISEMS_ERROR("Can not create range with step size of 0");
    ...
    for(float i = begin; i <= end; i = i+step)
    {
        data.push_back(step);
    }
    ...
    for(int i = 0; i < data.size(); i++)
    {
        array->SetValue(i,new float(data[i]));
    }

    return array;
}
CScriptArray * AS_RangeD(double begin, double end, double step)
{
    if( step == 0.0)
    {
        LISEMS_ERROR("Can not create range with step size of 0");
    ...
    for(int i = 0; i < data.size(); i++)
    {
        array->SetValue(i,new double(data[i]));
    }
...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
    ...
    r = engine->RegisterObjectMethod("array<T>", "void Append(const T&in value)", asMETHOD(CScriptArray, InsertLast), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("array<T>", "void Prepend(const T&in value)", asMETHOD(CScriptArray, InsertFirst), asCALL_THISCALL); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("array<T>", "void Shuffle()", asMETHOD(CScriptArray, Shuffle), asCALL_THISCALL); assert( r >= 0 );
    ...
    int TypeId = engine->GetTypeInfoByName("array")->GetTypeId();
    TypeId = engine->GetTypeIdByDecl("array<T>");
    asITypeInfo * asi = engine->GetTypeInfoById(TypeId);
    if(asi != nullptr)
    {
    }
    LSMScriptEngine *sphengine = (LSMScriptEngine *)  engine->GetUserData();
    sphengine->RegisterTypeDebugTextConversion(TypeId,[sphengine](void*par){return par == nullptr? "array{null}": QString("array{") + QString::number(((CScriptArray *)(par))->GetSize()) + sphengine->GetTypeName(((CScriptArray *)(par))->GetElementTypeId())+"}";});
    sphengine->RegisterTypeDebugList(TypeId
                    ,[sphengine](void*par){return par == nullptr? std::vector<int>({0}):std::vector<int>({(int)(((CScriptArray *)(par))->GetSize())});}
                    ,[sphengine](void*par){return par == nullptr? 0:(int)(((CScriptArray *)(par))->GetElementTypeId());}
                    ,[sphengine](void*par, std::vector<int> index){return par == nullptr? nullptr:(((CScriptArray *)(par))->At(index[0]));}
    );
}

CScriptArray &CScriptArray::operator=(const CScriptArray &other)
{
    std::cout << "assign script array " << std::endl;
...
void CScriptArray::SetValueNoRefAdd(asUINT index, void *value)
{
    ...
    {
        void*tmp = ptr;
        void **d = (void**)(buffer->data + index * sizeof(void*));
        *d = value;
...
void CScriptArray::SetValue(asUINT index, void *value, bool assign)
{
	// At() will take care of the out-of-bounds checking, though
	// if called from the application then nothing will be done
	void *ptr = At(index);
...
void CScriptArray::Shuffle()
{
    ...
    if( size > 1 )
    {
        ...
        for( asUINT i = 0; i < size ; i++ )
        {
            ...
            {
                Copy(TEMP, GetArrayItemPointer(i));
                Copy(GetArrayItemPointer(i), GetArrayItemPointer(indices[i]));
                Copy(GetArrayItemPointer(indices[i]), TEMP);
            }
        }
    }
}
...
void CScriptArray::CopyBuffer(SArrayBuffer *dst, SArrayBuffer *src)
{
	...
	if( subTypeId & asTYPEID_OBJHANDLE )
	{
        std::cout << "copy buffer handle type "<< std::endl;
	...
	{
        std::cout << "copy buffer object type "<< std::endl;
...
void CScriptArray::AddRef() const
{
    ...
    std::cout << "add array ref " << refCount << std::endl;
...
void CScriptArray::Release() const
{
	...
	gcFlag = false;
    std::cout << "release array ref " << refCount << std::endl;
