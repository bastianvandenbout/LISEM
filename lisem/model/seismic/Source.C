...
void Source::prepareTimeFunc(bool doFilter, double sw4TimeStep, int sw4TimeSamples, SeisFilter* sw4_filter)
{
   ...
   if (mTimeDependence == iDiscrete || mTimeDependence == iDiscrete6moments )
   {
      ...
      else
      {
         ...
         SeisFilter my_filter( sw4_filter->get_type(), sw4_filter->get_order(), sw4_filter->get_passes(),
...
//-----------------------------------------------------------------------
void Source::filter_timefunc( SeisFilter* filter_ptr, double tstart, double dt, int nsteps )
{
   if( !m_is_filtered )
   {
