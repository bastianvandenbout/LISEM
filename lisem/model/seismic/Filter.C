...
SeisFilter::SeisFilter(SeisFilterType type, unsigned int numberOfPoles, unsigned int numberOfPasses, double f1, double f2)
{
  m_type = type;
...
// destructor
SeisFilter::~SeisFilter()
{
  for (int q=0; q<m_numberOfSOS; q++)
    delete m_SOSp[q];
}

void SeisFilter::computeSOS(double dt)
{
  m_dt = dt;
...
ostream& operator<<( ostream& output, const SeisFilter& s )
{
  if (s.m_type == lowPass)
    output << "Lowpass";
  else
...
double SeisFilter::complexConjugatedPolesBP(double f1, double f2, double dt, double alpha, 
...
double SeisFilter::complexConjugatedPolesLP(double fc, double dt, double alpha, 
...
void SeisFilter::a2d(double n[3], double d[3], Polynomial &b, Polynomial &a)
{
// transform analog to digital by the transformation AD(s) = (1-s)/(1+s)
...
//
double SeisFilter::estimatePrecursor()
{
...
//
void SeisFilter::evaluate(int N, double *u, double *mf)
