...
friend std::ostream& operator<<(std::ostream& output, const SeisFilter& s);

public:
SeisFilter(SeisFilterType type, unsigned int numberOfPoles, unsigned int numberOfPasses, double f1, double f2);
~SeisFilter();
void evaluate(int N, double *u, double *mf);
void computeSOS(double dt);
double estimatePrecursor();

SeisFilterType get_type(){return m_type;}
