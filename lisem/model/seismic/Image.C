...
int SeisImage::mPreceedZeros=0;

int SeisImage::MODES=40;
...
void SeisImage::associate_gridfiles( vector<SeisImage*>& imgs )
{
   ...
   if( mEW->topographyExists() && (mLocationType == SeisImage::X || mLocationType == SeisImage::Y) &&
       !(mMode == SeisImage::GRIDX || mMode == SeisImage::GRIDY || mMode == SeisImage::GRIDZ) )
   {
   ...
   // Require same precision for now, simplifies file format.
      ...
      {
         if( mLocationType == SeisImage::X && imgs[i]->mMode == SeisImage::GRIDZ && imgs[i]->mLocationType == SeisImage::X &&
				       fabs(mCoordValue-imgs[i]->mCoordValue) < 1e-3 && m_double == imgs[i]->m_double )
	    m_gridimage = imgs[i];
	 if( mLocationType == SeisImage::Y && imgs[i]->mMode == SeisImage::GRIDZ && imgs[i]->mLocationType == SeisImage::Y &&
	     fabs(mCoordValue-imgs[i]->mCoordValue) < 1e-3 && m_double == imgs[i]->m_double )
	    m_gridimage = imgs[i];
      }
      if( m_gridimage == SeisImage::nil )
      {
         m_gridimage = new SeisImage( mEW, 0.0, 0.0, 0, 0, mFilePrefix, SeisImage::GRIDZ, mLocationType, mCoordValue,
				  m_double, false );
...
//-------------------------------------
void SeisImage::setSteps(int a_steps)
{
  char buffer[50];
...
//-----------------------------------------------------------------------
void SeisImage::computeGridPtIndex()
{
...
//     printf("======== Initializing Image ==========\n");
  ...
  if (mLocationType == SeisImage::X || mLocationType == SeisImage::Y)
  { 
...
// curvilinear grid on top: copy location from top Cartesian grid
  ...
  else if (mLocationType == SeisImage::Z)
  {
...
//-----------------------------------------------------------------------
bool SeisImage::plane_in_proc( int a_gridIndexCoarsest)
{
...
//-----------------------------------------------------------------------
const std::string SeisImage::fieldSuffix(ImageMode mode) const
{
...
//-----------------------------------------------------------------------
bool SeisImage::is_time_derivative() const
{
   return mMode == SeisImage::DIVDT    || mMode == SeisImage::CURLMAGDT || 
          mMode == SeisImage::MAGDUDT  || mMode == SeisImage::HMAGDUDT  || 
          mMode == SeisImage::HMAXDUDT || mMode == SeisImage::VMAXDUDT;
}

//-----------------------------------------------------------------------
bool SeisImage::timeToWrite(double time, int cycle, double dt )
{
  // -----------------------------------------------
...
//-----------------------------------------------------------------------
bool SeisImage::timeToWrite( int cycle )
{
  // -----------------------------------------------
  ...
  if (iwrite)
    {
      ...
      bool breakLoop      = (mLocationType == SeisImage::Z);
...
//-----------------------------------------------------------------------
void SeisImage::computeImageQuantity(std::vector<Sarray> &a_mu, int a_nComp )
{
  ASSERT(m_isDefinedMPIWriters);
// plane_in_proc returns true for z=const lpanes, because all processors have a part in these planes
  bool iwrite   = plane_in_proc(m_gridPtIndex[0]);
...
//-----------------------------------------------------------------------
void SeisImage::computeImageGrid( Sarray &a_X, Sarray &a_Y, Sarray &a_Z )
{
  ASSERT(m_isDefinedMPIWriters);
  ASSERT( mMode == SeisImage::GRIDX ||mMode == SeisImage::GRIDY ||mMode == SeisImage::GRIDZ );
  if (plane_in_proc(m_gridPtIndex[0]))
  {
     int topCartesian = mEW->mNumberOfCartesianGrids - 1;
     int component = 1;
     if( mMode == SeisImage::GRIDY )
	component = 2;
     else if( mMode == SeisImage::GRIDZ )
	component = 3;
...
//-----------------------------------------------------------------------
void SeisImage::computeImageLatLon(Sarray &a_X, Sarray &a_Y, Sarray &a_Z ) 
{
   ASSERT(m_isDefinedMPIWriters);
   ASSERT( mMode == SeisImage::LAT || mMode == SeisImage::LON );
// plane_in_proc returns true for z=const lpanes, because all processors have a part in these planes
   if (plane_in_proc(m_gridPtIndex[0]))
   {
...
//-----------------------------------------------------------------------
void SeisImage::computeImagePvel(std::vector<Sarray> &mu, std::vector<Sarray> &lambda,
			     std::vector<Sarray> &rho )
{
   ASSERT(m_isDefinedMPIWriters);
   ASSERT( mMode == SeisImage::P );
   if (plane_in_proc(m_gridPtIndex[0]))
   {
      int gmin, gmax;
      if( mLocationType == SeisImage::Z )
...
//-----------------------------------------------------------------------
void SeisImage::computeImageSvel(std::vector<Sarray> &mu, std::vector<Sarray> &rho )
{
   ASSERT(m_isDefinedMPIWriters);
   ASSERT( mMode == SeisImage::S );
   if (plane_in_proc(m_gridPtIndex[0]))
   {
      int gmin, gmax;
      if( mLocationType == SeisImage::Z )
...
//-----------------------------------------------------------------------
void SeisImage::copy2DArrayToImage(Sarray &u2)
{
   ASSERT(m_isDefinedMPIWriters);
...
// plane_in_proc returns true for z=const planes, because all processors have a part in these planes
   ...
   if (iwrite)
   {
      ASSERT2(mLocationType == SeisImage::Z, "Image::copy2DArrayToImage only works for z=const");
// write the data...
...
//-----------------------------------------------------------------------
void SeisImage::writeImagePlane_2(int cycle, std::string &path, double t )
{
   if( !m_user_created )
      return;
...
// plane_in_proc returns true for z=const lpanes, because all processors have a part in these planes
  ...
  // offset initialized to header size:
   ...
   if( iwrite )
   {
      if( mMode == SeisImage::GRIDX || mMode == SeisImage::GRIDY || mMode == SeisImage::GRIDZ )
   ...
   if( m_pio[0]->proc_zero() )
   {
	 ...
	 cout << "ERROR: Image::writeImagePlane_2 could not write time" << endl;
      ...
      if( mLocationType == SeisImage::X )
         ltype = 0;
      else if( mLocationType == SeisImage::Y )
	 ltype = 1;
      else
	 ltype = 2;
...
//-----------------------------------------------------------------------
void SeisImage::add_grid_to_file( const char* fname, bool iwrite, size_t offset )
{
   bool ihavearray = plane_in_proc(m_gridPtIndex[0]);
   if( ihavearray )
   {
...
//-----------------------------------------------------------------------
void SeisImage::add_grid_filenames_to_file( const char* fname )
{
   if( m_pio[0]->proc_zero() )
   {
      stringstream str1;
...
//-----------------------------------------------------------------------
void SeisImage::update_maxes_vVelMax(std::vector<Sarray> &a_Up, std::vector<Sarray> &a_Um, double dt )
{
...
//-----------------------------------------------------------------------
void SeisImage::update_maxes_vMax(std::vector<Sarray> &a_U )
{
...
//-----------------------------------------------------------------------
void SeisImage::computeDivergence( std::vector<Sarray> &a_U, std::vector<double*>& a_div )
{
   ASSERT(m_isDefinedMPIWriters);
...
//-----------------------------------------------------------------------
void SeisImage::computeCurl( std::vector<Sarray>& a_U, std::vector<double*>& a_curl )
{
   ASSERT(m_isDefinedMPIWriters);
   ASSERT( a_U.size()    == mEW->mNumberOfGrids )
...
//-----------------------------------------------------------------------
bool SeisImage::needs_mgrad() const
{
