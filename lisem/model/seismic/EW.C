			...
			vector<Source*> & a_sources, int dminus )
{
   ...
   {
      SeisImage* img = mImageFiles[fIndex];

      if( img->mMode == SeisImage::HMAXDUDT)
      {
	 if( dminus )
      ...
      if( img->mMode == SeisImage::HMAX )
	    img->update_maxes_hMax( a_Up );

      if( img->mMode == SeisImage::VMAXDUDT)
      {
	 if( dminus )
	 ...
	 td = img->is_time_derivative();
      ...
      {
	 if(img->mMode == SeisImage::UX ) 
	    img->computeImageQuantity(a_Up, 1);
	 else if(img->mMode == SeisImage::UY )
	    img->computeImageQuantity(a_Up, 2);
	 else if(img->mMode == SeisImage::UZ )
	 ...
	 else if(img->mMode == SeisImage::RHO )
	    img->computeImageQuantity(a_Rho, 1);
	 else if(img->mMode == SeisImage::MU )
	    img->computeImageQuantity(a_Mu, 1);
	 else if(img->mMode == SeisImage::LAMBDA )
	    img->computeImageQuantity(a_Lambda, 1);
	 else if(img->mMode == SeisImage::QP )
	    img->computeImageQuantity(mQp, 1);
	 else if(img->mMode == SeisImage::QS )
	    img->computeImageQuantity(mQs, 1);
	 else if(img->mMode == SeisImage::P )
	    img->computeImagePvel(a_Mu, a_Lambda, a_Rho);
	 else if(img->mMode == SeisImage::S )
	    img->computeImageSvel(a_Mu, a_Rho);
	 else if(img->mMode == SeisImage::DIV || img->mMode == SeisImage::DIVDT 
		 || img->mMode == SeisImage::CURLMAG || img->mMode == SeisImage::CURLMAGDT )
	    img->computeImageDivCurl( a_Up, a_U, a_Um, mDt, dminus );
	 else if(img->mMode == SeisImage::LAT || img->mMode == SeisImage::LON )
	    img->computeImageLatLon( mX, mY, mZ );
	 else if(img->mMode == SeisImage::TOPO )
	 {
	    if (topographyExists())
	       img->copy2DArrayToImage(mTopo); // save the raw topography; the smoothed is saved by the mode=grid with z=0
	 }
	 else if( img->mMode == SeisImage::UZEXACT || img->mMode == SeisImage::UXEXACT ||
		  img->mMode == SeisImage::UYEXACT || img->mMode == SeisImage::UXERR   ||
		  img->mMode == SeisImage::UYERR   || img->mMode == SeisImage::UZERR   )
	 {
	    ...
	    exactSol( time, Uex, alpha, a_sources );
            if( img->mMode == SeisImage::UXERR || img->mMode == SeisImage::UYERR || img->mMode == SeisImage::UZERR )
	    {
	       for( int g=0 ; g < mNumberOfGrids ; g++ )
	       {
	    ...
	    if( img->mMode == SeisImage::UXEXACT || img->mMode == SeisImage::UXERR )
	       img->computeImageQuantity(Uex,1);
	    else if( img->mMode == SeisImage::UYEXACT || img->mMode == SeisImage::UYERR )
	       img->computeImageQuantity(Uex,2);
	    else if( img->mMode == SeisImage::UZEXACT || img->mMode == SeisImage::UZERR )
            ...
            if( m_use_attenuation )
	    {
         ...
         else if( img->mMode == SeisImage::GRIDX || img->mMode == SeisImage::GRIDY || img->mMode == SeisImage::GRIDZ )
	    img->computeImageGrid(mX, mY, mZ );
         else if(img->mMode == SeisImage::MAGDUDT )
	 {
            if( dminus )
	 ...
	 else if(img->mMode == SeisImage::HMAGDUDT )
	 {
            if( dminus )
         ...
         else if(img->mMode == SeisImage::MAG )
	    img->computeImageMag( a_Up );
         else if(img->mMode == SeisImage::HMAG )
...
//	      || !img->mMode == Image::HMAX   || !img->mMode == Image::VMAX )
	 else if( !(   img->mMode == SeisImage::HMAXDUDT || img->mMode == SeisImage::VMAXDUDT
		|| img->mMode == SeisImage::HMAX   || img->mMode == SeisImage::VMAX
		|| img->mMode == SeisImage::GRADRHO || img->mMode == SeisImage::GRADMU || img->mMode == SeisImage::GRADLAMBDA
		       || img->mMode == SeisImage::GRADP || img->mMode == SeisImage::GRADS ) )
      {
	if (proc_zero())
	{
...
//-----------------------------------------------------------------------
void EW::addImage(SeisImage* i)
{
  mImageFiles.push_back(i);
}

//-----------------------------------------------------------------------
void EW::addImage3D(SeisImage3D* i)
{
...
{
   // Image planes
   SeisImage::setSteps(mNumberOfTimeSteps);
   for (unsigned int fIndex = 0; fIndex < mImageFiles.size(); ++fIndex)
   {
   ...
   for (unsigned int fIndex = 0; fIndex < mImageFiles.size(); ++fIndex)
      if( mImageFiles[fIndex]->mMode == SeisImage::GRIDX
       || mImageFiles[fIndex]->mMode == SeisImage::GRIDY
       || mImageFiles[fIndex]->mMode == SeisImage::GRIDZ )
	 mImageFiles[fIndex]->computeImageGrid(mX, mY, mZ );

   // Volume images
   SeisImage3D::setSteps(mNumberOfTimeSteps);
   for (unsigned int fIndex = 0; fIndex < mImage3DFiles.size(); ++fIndex)
...
//-----------------------------------------------------------------------
void EW::set_prefilter( SeisFilterType passband, int order, int passes, double fc1, double fc2 )
{
  m_prefilter_sources = true;
// we could build the filter object right here...
  m_filter_ptr    = new SeisFilter( passband, order, passes, fc1, fc2);
  m_filterobs_ptr = new SeisFilter( passband, order, passes, fc1, fc2);
}
...
// rudimentary checks
   if ((prec==4 || prec==8) && npatches==1 && plane==2 && mode==SeisImage::TOPO)
   {
     if (proc_zero())
