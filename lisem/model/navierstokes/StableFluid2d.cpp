...
StableFluid2d::StableFluid2d(int countX, int countY, float width, float height)
{
    ...
    _block = new int[length]();
    _blockm = new int[length]();
...
void StableFluid2d::SetBounds(const int b, float* d, int c = 0)
{
    bool confine_edges = false;
    ...
    for (int y = 0; y < resY; y++)
    {
        ...
        {
            //it is in or outside blocking

            bool in = _block[IX(y,x)];
            bool in_x1 = true;
            bool in_x2 = true;
            bool in_y1 = true;
            bool in_y2 = true;
            ...
            if(x> 1)
            {
               in_x1 = _block[IX(y,x-1)];
               val_x1 = d[IX(y,x-1)];
            }
            if(x< resX-1)
            {
               in_x2 = _block[IX(y,x+1)];
               val_x2 = d[IX(y,x+1)];
            }
            if(y> 1)
            {
               in_y1 = _block[IX(y-1,x)];
               val_y1 = d[IX(y-1,x)];
            }
            if(y< resY-1)
            {
               in_y2 = _block[IX(y+1,x)];
               val_y2 = d[IX(y+1,x)];
            }
            ...
            if(in)
            {
                if(b >0)
                {
                    ...
                    if(b == 1)
                    {
                        if(!in_x1)
                        {
                            valn = std::max(valn,0.0);
                        }

                        if(!in_x2)
                        {
                            valn = std::min(valn,0.0);
                        }
                    }else if(b == 2)
                    {
                        if(!in_y1)
                ...
                if(c == 0)
                {
                    ...
                    if(in_x1)
                    {
                        valn = std::max(val_x1,0.0);
                    }

                    if(in_x2)
                    {
                        valn = std::max(val_x2,0.0);
                    }

                    if(in_y1)
                    {
                        valn = std::max(val_y1,0.0);
                    }
                    ...
                    d[IX(y,x)] = std::max({val_x1,val_x2,val_y1,val_y2});
                }
            }
...
void StableFluid2d::Project(float * u, float * v, float * p, float * div)
{
    ...
    SetBounds(0, div); SetBounds(0, p,1);
