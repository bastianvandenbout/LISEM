...
private:
    ...
    int * _block; ///< blocking cells
    int * _blockm; ///< blocking cells
    ...
    /// \param c zero if not pressure
    void SetBounds(const int b, float * d, int c);
...
public:
    ...
    int* Block() { return _block; };					///< returns velocity x component values
