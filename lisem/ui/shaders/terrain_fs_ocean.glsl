...
highp float GetElevation(highp vec4 wpos)
{
    ...
    return ZScale* 10.0 * texture(TextureH,texcoord+ vec2(1.3*iTime,-0.2*iTime)).r + ZScale*15.0 * texture(TextureH,vec2(texcoord.y,texcoord.x)/2.0 + vec2(iTime,-iTime)).r;
}
...
void main() {
    ...
    //vec3 normal = normalize(vec3(water(wpos.xz-xdiff) - water(wpos.xz+xdiff), 1.0, water(wpos.xz-ydiff) - water(wpos.xz+ydiff)));

    vec3 normaladd = GetNormal(VIN.worldPosition);
    vec3 normal = normalize((normaladd.z) * normals + normaladd.x * binormals + normaladd.y * tanormals);
    ...
    float reflec_coeff = 1.0-clamp(dot(reflect_coord,vec3(0.0, 1.0, 0.0)),0.0,1.0);
    ...
    vec3 reflskycolor= 2.0 * vec3(0.1, 0.15, 0.20);


    vec3 finalcolor = vec3(reflskycolor * skycolor.rgb * reflec_coeff + (1.0-reflec_coeff) * watercolor );

    float sundot = clamp(dot(reflect_coord,iSunDir),0.0,1.0);

    // Sun
    float wsunrefl = 1.0 * reflec_coeff*(0.5*pow( sundot, 10.0 )+0.25*pow( sundot, 3.5)+.75*pow( sundot, 300.0));
    ...
    //frag_colour = vec4(0.0,0.0,0.5 + sin((VIN.worldPosition.x + VIN.worldPosition.z)/3.0),0.5) ;
