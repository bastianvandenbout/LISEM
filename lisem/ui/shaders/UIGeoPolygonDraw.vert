...
layout(location = 4) in float attr;
...
void main() {
    ...
    }else
    {
        ...
        if(is_relative > 0)
        {
            ...
            if(difnorm > 1e-20)
            {
                pos_scaled = posorgn.xy + (dif/difnorm)*relative_size*(vec2(2.0/ug_windowpixsizex,2.0/ug_windowpixsizey));
            }else
            {
    ...
    gl_Position = posn;
    Out.Position = vec4(posn.x,posn.y,0.0,0.0);
